...
function useBreakpoint(): Breakpoint {
  ...
  useEffect(() => {
    const update = () => {
      ...
      if (width < 768) setBp("mobile");
      else if (width < 1024) setBp("tablet");
      else setBp("desktop");
    };
    update();
...
function getGlobeCardStyle(relativeIndex: number, breakpoint: Breakpoint): CSSProperties {
  const configs = {
    desktop: { sideX: 330, backX: 560, sideY: 45, backY: 105, sideRotate: 32, backRotate: 48, sideScale: 0.86, backScale: 0.68, backOpacity: 0.32 },
    tablet: { sideX: 245, backX: 400, sideY: 40, backY: 90, sideRotate: 28, backRotate: 42, sideScale: 0.78, backScale: 0.64, backOpacity: 0.18 },
    mobile: { sideX: 155, backX: 260, sideY: 24, backY: 60, sideRotate: 16, backRotate: 28, sideScale: 0.72, backScale: 0.5, backOpacity: 0 },
  } as const;
  ...
  switch (relativeIndex) {
    case 0:
      return { transform: "translateX(-50%) translateY(0px) translateZ(0px) rotateY(0deg) scale(1)", opacity: 1, zIndex: 50, filter: "blur(0px)", pointerEvents: "auto" };
    case -1:
      return { transform: `translateX(calc(-50% - ${c.sideX}px)) translateY(${c.sideY}px) rotateY(${c.sideRotate}deg) scale(${c.sideScale})`, opacity: breakpoint === "mobile" ? 0.45 : 0.78, zIndex: 40, filter: "blur(0px)", pointerEvents: breakpoint === "mobile" ? "none" : "auto" };
    case 1:
      return { transform: `translateX(calc(-50% + ${c.sideX}px)) translateY(${c.sideY}px) rotateY(-${c.sideRotate}deg) scale(${c.sideScale})`, opacity: breakpoint === "mobile" ? 0.45 : 0.78, zIndex: 40, filter: "blur(0px)", pointerEvents: breakpoint === "mobile" ? "none" : "auto" };
    case -2:
      return { transform: `translateX(calc(-50% - ${c.backX}px)) translateY(${c.backY}px) rotateY(${c.backRotate}deg) scale(${c.backScale})`, opacity: c.backOpacity, zIndex: 25, filter: c.backOpacity > 0 ? "blur(1px)" : "blur(2px)", pointerEvents: "none" };
    case 2:
      return { transform: `translateX(calc(-50% + ${c.backX}px)) translateY(${c.backY}px) rotateY(-${c.backRotate}deg) scale(${c.backScale})`, opacity: c.backOpacity, zIndex: 25, filter: c.backOpacity > 0 ? "blur(1px)" : "blur(2px)", pointerEvents: "none" };
    default:
      return { transform: "translateX(-50%) translateY(160px) scale(0.5)", opacity: 0, zIndex: 0, filter: "blur(3px)", pointerEvents: "none" };
  }
}

export function PublicEventsGlobeCarousel({
  events,
  viewDetailsLabel,
}: {
  events: PublicEvent[];
    ...
    <div className="relative mt-10 w-full max-w-full overflow-x-hidden sm:mt-14" dir="ltr">
      <div
        ...
        className={`relative mx-auto w-full max-w-full overflow-hidden [perspective:1400px] sm:[perspective:1800px] ${
          breakpoint === "mobile" ? "h-[680px] touch-none cursor-grab active:cursor-grabbing" : breakpoint === "tablet" ? "h-[760px] touch-none cursor-grab active:cursor-grabbing" : "h-[840px] touch-none cursor-grab active:cursor-grabbing lg:h-[860px]"
        }`}
      >
        <div
          ...
          return (
            ...
            >
              <PublicEventCard
                ...
                ctaHref={viewDetailsLabel ? `/events/${event.slug}` : undefined}
              />
            </div>
          );
        })}
